...
module.exports = function toString(ast) {
  ...
  if (ast.term_min) {
    if (ast.term_min === implicit || ast.term_max === implicit) {
      result += ast.term_min === implicit ? '<' : '>';
      result += (ast.min_inclusive === true && ast.term_min !== implicit) || (ast.max_inclusive === true && ast.term_max !== implicit) ? '=' : '';
      result += ast.term_min === implicit ? ast.term_max : ast.term_min;
    } else {
      if (ast.min_inclusive) {
        result += '[';
      } else {
        result += '{';
      }
