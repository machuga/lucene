...
module.exports = function toString(ast) {
  ...
  if (ast.termMin) {
    if (ast.termMin === implicit || ast.termMax === implicit) {
      result += ast.termMin === implicit ? '<' : '>';
      result += (ast.minInclusive === true && ast.termMin !== implicit) || (ast.maxInclusive === true && ast.termMax !== implicit) ? '=' : '';
      result += ast.termMin === implicit ? ast.termMax : ast.termMin;
    } else {
      if (ast.minInclusive) {
        result += '[';
      } else {
